...
Sink block. Tap the output of digital_ofdm_frame_equalizer_vcvc_1
(post-equalization OFDM vectors of length fft_len). Frames are
delimited by packet_num and frame_len stream tags placed on the
first OFDM symbol of each frame by the demux. For every frame,
extracts the pilot subcarrier values from every OFDM symbol,
averages them, and saves to a .npz file alongside packet_num.

The demux only emits payload symbols for headers that successfully
parsed, so all frames seen here correspond to validated frames -
no extra validation is needed.

Offline, combine with chan_taps_logger output by packet_num and
compute SNR per frame:
    n_eq = mean_pilots - known_pilots
    sigma2 = mean(|n_eq|^2)
    snr_post_eq = 1.0 / sigma2
"""
...
class blk(gr.sync_block):
    ...
    def _finalize_frame(self):
        ...
        with self._lock:
            self.pn_list.append(int(self._curr_pn))
